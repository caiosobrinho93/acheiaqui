      ...
      {isOpen && (
        <div className="fixed inset-0 z-[200] overflow-hidden">
          ...
          <motion.div
            ...
            className="absolute top-0 right-0 h-full w-full max-w-[450px] bg-surface border-l border-white/5 shadow-2xl flex flex-col"
          >
            {/* Header */}
            <div className="p-8 border-b border-white/5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary shadow-neon-soft">
              ...
              <button 
                onClick={onClose}
                className="h-10 w-10 rounded-xl bg-white/5 flex items-center justify-center hover:bg-white/10 transition-all text-text-muted hover:text-foreground"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
              ...
              <div className="relative group">
                ...
                <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
                   {query && (
                     <button 
                       onClick={() => setQuery("")}
                       className="h-8 w-8 rounded-lg bg-white/10 flex items-center justify-center hover:bg-white/20 transition-all text-text-muted hover:text-foreground mr-1"
                     >
                       <X className="h-4 w-4" />
                     </button>
                   )}
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {results.map((product) => (
                    <button
                      ...
                      className="flex items-center gap-4 p-3 rounded-2xl hover:bg-white/5 border border-transparent hover:border-white/5 transition-all group text-left"
                    >
                  ...
                  {results.length === 0 && (
                    <div className="py-20 flex flex-col items-center gap-6 text-center">
                      ...
                      <p className="text-xs font-black uppercase tracking-widest text-text-muted italic">Protocolo não encontrado.<br/>Tente outros termos.</p>
                    </div>
                  )}
                </div>
              ) : (
                <div className="space-y-10 py-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {[
                        "Tente buscar por marcas específicas",
                        "Busque por categorias (ex: Air Fryer)",
                        "Palavras-chave como 'Gamer' ou 'Cozinha'",
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {['Black Friday', 'Eletrodomésticos', 'Setup Gamer', 'Mais Vendidos'].map((tag) => (
                        <button 
                          key={tag}
                          onClick={() => setQuery(tag)}
                          className="px-4 py-2 rounded-lg bg-white/5 border border-white/10 hover:border-primary/40 hover:bg-primary/10 hover:text-primary transition-all text-xs font-black uppercase tracking-widest"
                        >
                          {tag}
                        </button>
                      ))}
